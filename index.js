...
${gg} ${exists ? " exists " : "*[ RIBASBOT ]* ðŸ’•"}`, MessageType.text)
}
...
conn.sendMessage(id, 'Aku BOT nya RIBAS' ,MessageType.text);
}
...
if (text.includes("!bmkg")){
  const ribas = text.replace(/!infogempa /, "")
...
if (text.includes("!igstalk")){
  const ribas = text.replace(/!igstalk /, "")
  axios.get(`https://alfians-api.herokuapp.com/api/stalk?username=${ribas}`).then ((res) =>{
  conn.sendMessage(id, '[ WAIT ] Sedang di prosesâ³ silahkan tunggu sebentar', MessageType.text)
  let hasil = `Info Instagram *_${ribas}_* \n\n *Username* : _${res.data.Username}_ \n *Nama ï¸* : _${res.data.Name}_. \n *Jumlah Followers ï¸* : _${res.data.Jumlah_Followers}_ \n *Jumlah Following ï¸* : _${res.data.Jumlah_Following}_ \n *Jumlah Post ï¸* : _${res.data.Jumlah_Post}_ `;
  conn.sendMessage(id, hasil, MessageType.text);
})
}
if (text.includes("!infoanime")){
const ribas = text.replace(/!infoanime /, "")
axios.get(`https://arugaz.herokuapp.com/api/dewabatch?q=${ribas}`).then((res) => {
conn.sendMessage(id, '[ WAIT ] Sedang di prosesâ³ silahkan tunggu sebentar', MessageType.text)
    let hasil = ` *Info Anime* ${ribas} : \n\n ${res.data.result} `;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
if (text.includes("!animek")){
const ribas = text.replace(/!infoanime /, "")
axios.get(`https://mhankbarbars.herokuapp.com/api/kuso?q=${ribas}`).then((res) => {
...
const ribas = text.replace(/!covidcountry /, "")
axios.get(`https://arugaz.herokuapp.com/api/corona?country=${ribas}`).then((res) => {
conn.sendMessage(id, '[ WAIT ] Sedang di prosesâ³ silahkan tunggu sebentar', MessageType.text)
...
const ribas = text.replace(/!ping /, "")
axios.get(`https://api.banghasan.com/domain/nping/${ribas}`).then((res) => {
    let hasil = `*query : ${res.data.query}*\n\nresult : ${res.data.hasil}`;                                        
          conn.sendMessage(id, hasil ,MessageType.text);
  })
 }                                                                                                                     
  if (text.includes("!hostsearch")){
const ribas = text.replace(/!hostsearch /, "")
axios.get(`https://api.banghasan.com/domain/hostsearch/${ribas}`).then((res) => {
    let hasil = `*query : ${res.data.query}*\n\nhasil : ${res.data.hasil}`;
    conn.sendMessage(id, hasil ,MessageType.text);
  })
 }
if (text.includes("!zodiak")){
const ribas = text.replace(/!zodiak /, "")
axios.get(`https://arugaz.herokuapp.com/api/getzodiak?nama=aruga&tgl-bln-thn=${ribas}`).then((res) => {
...
const ribas = text.replace(/!simi /, "")
axios.get(`https://st4rz.herokuapp.com/api/simsimi?kata=${ribas}`).then((res) => {
...
if (text.includes("!checkip"))
  { const ribas = text.replace(/!checkip /, "")
  axios.get(`https://mnazria.herokuapp.com/api/check?ip=${ribas}`).then((res) =>{
...
if (text.includes('!waifu')){                                                                        var ribas = text.replace(/!waifu /, '')
...
//Tolonglah bro jangan di ubah ubah RibasYT
